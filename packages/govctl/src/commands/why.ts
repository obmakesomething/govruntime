...
export function registerWhy(program) {
    program
        ...
        const opening = caseEvents.filter((e) => e.event_type === "case_opened" || e.event_type === "ticket_issued");
        const refinements = caseEvents.filter((e) => e.event_type === "ticket_reissued" ||
        ...
        const pauses = caseEvents.filter((e) => e.event_type === "ticket_paused" || e.event_type === "workstream_deferred");
        if (opening.length > 0) {
