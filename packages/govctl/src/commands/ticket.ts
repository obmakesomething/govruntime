...
export function registerTicket(program: Command): void {
  ...
  ticketCmd
    ...
    .action((opts: { cwd: string; status?: string }) => {
      ...
      for (const t of tickets) {
        const isActive =
          state.active_ticket?.ticket_id === t.ticket_id;
        ...
        console.log(
          `${marker}${chalk.bold(t.ticket_id)}  ${statusStr}  ${wsStr}`
        );
        console.log(`   ${chalk.gray("Title:")} ${t.title}`);
        console.log(
          `   ${chalk.gray("Case:")} ${t.case_id}  ${chalk.gray("Rev:")} ${t.revision}  ${chalk.gray("Criteria:")} ${t.acceptance_criteria.length}`
        );
        console.log("");
      }
    });
  ...
  ticketCmd
    ...
    .option("--cwd <path>", "Working directory", process.cwd())
    .action((ticketId: string, opts: { cwd: string }) => {
      const cwd = path.resolve(opts.cwd);
      const state = loadState(cwd);
      ...
      console.log(
        `    Ambiguity: ${pct(r.ambiguity)}  Scope Drift: ${pct(r.scope_drift)}  Blast: ${r.blast_radius}`
      );
      console.log("");
    });
  ...
  ticketCmd
    ...
    .action(
      (
        ticketId: string,
        opts: { reason: string; cwd: string; title?: string }
      ) => {
        const cwd = path.resolve(opts.cwd);
        const state = loadState(cwd);
        ...
        const reissueOpts: { reason: string; patch?: { title: string } } = {
          reason: opts.reason,
        };
        if (opts.title) reissueOpts.patch = { title: opts.title };
...
function ticketStatusColor(status: string): string {
  ...
  if (s === "in_progress" || s === "assigned") return chalk.green(status);
  if (s === "paused" || s === "blocked_by_conflict") return chalk.yellow(status);
  if (s === "superseded" || s === "cancelled") return chalk.red(status);
  if (s === "done") return chalk.gray(status);
  return chalk.white(status);
}

function workstreamColor(ws: string): string {
  if (ws === "ACTIVE") return chalk.green(ws);
  if (ws === "PAUSED" || ws === "STAYED" || ws === "BLOCKED") return chalk.yellow(ws);
  if (ws === "SUPERSEDED" || ws === "ABANDONED" || ws === "OVERRULED") return chalk.red(ws);
  if (ws === "DONE") return chalk.gray(ws);
