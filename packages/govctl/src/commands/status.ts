...
export function registerStatus(program: Command): void {
  program
    ...
    .action((opts: { cwd: string }) => {
      ...
      // Paused / Superseded
      const paused = state.tickets.filter(
        (t) =>
      ...
      if (paused.length > 0) {
        ...
        for (const t of paused) {
          console.log(
            `  ${chalk.gray(t.ticket_id)} — ${t.title}  [${statusColor(t.workstream_status)}]`
          );
        }
        console.log("");
      }
...
function computeWhy(cwd: string, state: ReturnType<typeof loadState>): string {
  if (!state.active_case) return "No active case. Governance context not established.";
  ...
  const deepen = caseEvents.filter(
    (e) => e.event_type === "workstream_deepened" || e.event_type === "ticket_reissued"
  );
