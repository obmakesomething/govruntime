...
import type { Evidence } from "@govruntime/govd";
...
export function registerEvidence(program: Command): void {
  ...
  evidenceCmd
    ...
    .action((opts: { cwd: string; limit: string; type?: string }) => {
      const cwd = path.resolve(opts.cwd);
      const state = loadState(cwd);
      const limit = parseInt(opts.limit, 10);

      const evidencePath = govPath(cwd, "evidence", "evidence.jsonl");
      let evidence = readJsonlFile<Evidence>(evidencePath);

      if (state.active_case) {
        evidence = evidence.filter(
          (e) => e.case_id === state.active_case?.case_id || !e.case_id
        );
      }
      ...
      for (const ev of evidence) {
        ...
        console.log(
          `  ${chalk.cyan(ev.evidence_id)}  ${tierColor(`[Tier ${ev.tier}]`)}  ${chalk.white(ev.type)}`
        );
        ...
        if (ev.claims.length > 0) {
          ...
          if (c) {
            console.log(
              `  ${chalk.gray("Claim:")} ${c.claim.slice(0, 120)} ${chalk.gray(`(${Math.round(c.confidence * 100)}%)`)}`
            );
          }
        }
        console.log("");
  ...
  evidenceCmd
    ...
    .action(
      (opts: {
        ...
        const admitOpts: Parameters<typeof admitUserStatement>[1] = {
          quote: opts.quote,
          claims: [{ claim: opts.claim ?? opts.quote, confidence: parseFloat(opts.confidence) }],
        };
        if (state.active_case) admitOpts.case_id = state.active_case.case_id;
        if (state.active_ticket) admitOpts.ticket_id = state.active_ticket.ticket_id;
