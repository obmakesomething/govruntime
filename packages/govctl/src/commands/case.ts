...
import type { Case } from "@govruntime/govd";

export function registerCase(program: Command): void {
  ...
  caseCmd
    ...
    .option("--cwd <path>", "Working directory", process.cwd())
    .action((caseId: string, opts: { cwd: string }) => {
      const cwd = path.resolve(opts.cwd);
      const state = loadState(cwd);
      ...
      for (const i of c.issue) console.log(`    - ${i}`);
      console.log("");
      if (c.judgment) {
  ...
  caseCmd
    ...
    .action(
      (opts: {
        ...
        console.log(
          chalk.cyan(
            `  govctl ticket create --area ARCH --seq 1 --title "..." --objective "..."`
          )
        );
        console.log("");
      }
  ...
  caseCmd
    ...
    .action((opts: { cwd: string }) => {
      ...
      for (const c of state.cases) {
        ...
        console.log(
          `${marker}${chalk.bold(c.case_id)}  ${chalk.gray(c.status)}  ${chalk.gray(c.opened_at.slice(0, 10))}`
        );
        console.log(`   ${c.issue[0] ?? "(no issue defined)"}`);
        console.log(
          `   Tickets: ${c.related_tickets.length}  Evidence: ${c.evidence.length}`
        );
        console.log("");
      }
    });
