...
export function registerTimeline(program) {
    program
        ...
        const targetCaseId = opts.case ?? state.active_case?.case_id;
        if (!targetCaseId) {
            console.log(chalk.yellow("\n  No active case. Run govctl init.\n"));
            return;
        }
        const docketPath = govPath(cwd, "docket", "docket_events.jsonl");
        let events = readJsonlFile(docketPath).filter((e) => e.case_id === targetCaseId);
        if (opts.type) {
            events = events.filter((e) => e.event_type.includes(opts.type));
        }
        events = events.slice(-limit);
        console.log(chalk.bold.cyan(`\n⚖️  Timeline — ${targetCaseId}\n`));
        console.log(chalk.gray(`  Showing ${events.length} event(s)${opts.type ? ` (type: ${opts.type})` : ""}\n`));
        if (events.length === 0) {
        ...
        for (const event of events) {
            ...
            console.log(`  ${icon}  ${eventColor(event.event_type.replace(/_/g, " "))}  ${chalk.gray(timeStr)}`);
            ...
            if (event.decision) {
                console.log(`     ${chalk.gray("Decision:")} ${event.decision.type} — ${event.decision.rationale}`);
            }
            console.log("");
        }
    });
