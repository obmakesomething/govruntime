...
import { ensureGovernanceDirs, touchJsonl, writeYamlFile, govPath, syncAgentRules, loadState, appendCleanStateEvent, } from "@govruntime/govd";
const CONSTITUTION = {
    version: "0.1",
    runtime: {
...
const STATUTES = {
    ...
    "path_literals.yaml": {
        id: "PATH-001",
        name: "document_literal_path_validation",
        version: "0.1",
        rule: "File path literals in tool inputs and governance documents must resolve to explicit, repository-relative or absolute paths before they are used as evidence or execution targets.",
        applies_to: { hook_events: ["PreToolUse", "UserPromptSubmit", "Stop"] },
        enforcement: {
            advisory: "warn",
            hard_block: "block",
            message: "Path literals must be syntactically valid and resolvable before use.",
        },
    },
};
const REGULATIONS = {
...
const REGO_POLICIES = {
    "govruntime.rego": `package govruntime.tool

import rego.v1

deny := array.concat(array.concat(scope.deny, destructive.deny), approval.deny)
warn := []
review := approval.review
`,
    "scope.rego": `package govruntime.tool.scope

import rego.v1

deny contains {
  "rule": "governance.no_active_case",
  "reason": "No active case is available.",
  "severity": "high"
} if {
  not input.governance.active_case
}

deny contains {
  "rule": "governance.no_active_ticket",
  "reason": "No active ticket is available.",
  "severity": "high"
} if {
  not input.governance.active_ticket
}

deny contains {
  "rule": "scope.forbidden_scope",
  "reason": sprintf("Path is forbidden for active branch: %s", [p]),
  "path": p,
  "severity": "critical"
} if {
  input.event.hook == "PreToolUse"
  some p in input.event.target_paths
  some pattern in input.governance.active_branch.forbidden_scope
  glob.match(pattern, ["/"], p)
}

deny contains {
  "rule": "scope.outside_intended_scope",
  "reason": sprintf("Path is outside approved ticket scope: %s", [p]),
  "path": p,
  "severity": "high"
} if {
  input.event.hook == "PreToolUse"
  some p in input.event.target_paths
  count(input.governance.active_branch.intended_scope) > 0
  not path_in_scope(p, input.governance.active_branch.intended_scope)
  not input.authorization.explicit_scope_expansion
}

path_in_scope(p, patterns) if {
  some pattern in patterns
  glob.match(pattern, ["/"], p)
}
`,
    "destructive.rego": `package govruntime.tool.destructive

import rego.v1

deny contains {
  "rule": "destructive.requires_explicit_authorization",
  "reason": sprintf("Destructive signal detected: %s", [s]),
  "severity": "critical"
} if {
  some s in input.event.destructive_signals
  not input.authorization.explicit_destructive_action
}
`,
    "approval.rego": `package govruntime.tool.approval

import rego.v1

deny contains {
  "rule": "repo.protected_path",
  "reason": sprintf("Path is protected and requires human approval: %s", [p]),
  "path": p,
  "severity": "critical"
} if {
  some p in input.event.target_paths
  some pattern in input.repo.protected_paths
  glob.match(pattern, ["/"], p)
  count(input.authorization.human_approval_ids) == 0
}

review contains {
  "rule": "repo.high_risk_path",
  "reason": sprintf("Path requires human review: %s", [p]),
  "path": p,
  "severity": "high"
} if {
  some p in input.event.target_paths
  some pattern in input.repo.high_risk_paths
  glob.match(pattern, ["/"], p)
  count(input.authorization.human_approval_ids) == 0
}
`,
        ...
        SessionStart: [
            {
                hooks: [
                    {
                        type: "command",
                        command: "govctl hook auto claude",
                    },
                ],
            },
        ],
...
const CODEX_HOOKS = {
    hooks: {
        exec: {
            command: "govctl hook auto codex",
            timeout: 10000,
        },
    },
};
export function registerInit(program) {
    program
        ...
        console.log(chalk.green("  ✓ statutes/ (11 statutes)"));
        // Regulations
        ...
        appendCleanStateEvent(cwd, {
            phase: "init",
            event: "governance_initialized",
            mode: CONSTITUTION.runtime.enforcement_mode,
            product_mode: CONSTITUTION.runtime.product_mode,
            namespace: CONSTITUTION.runtime.namespace,
            clean: true,
            created_at: new Date().toISOString(),
        });
        // Branch ledger
        writeYamlFile(govPath(cwd, "branches", "branch_ledger.yaml"), BRANCH_LEDGER_INITIAL);
        ...
        if (!fs.existsSync(claudeSettingsPath) || opts.force) {
            fs.writeFileSync(claudeSettingsPath, JSON.stringify(CLAUDE_SETTINGS, null, 2), "utf8");
        ...
        if (!fs.existsSync(codexHooksPath) || opts.force) {
            fs.writeFileSync(codexHooksPath, JSON.stringify(CODEX_HOOKS, null, 2), "utf8");
