    ...
    switch (event.hook_event_name) {
        ...
        case "PreToolUse": {
            ...
            if (result.decision === "block") {
                ...
                writeBlockResponse(result.reason ?? "Blocked by governance runtime.", result.applied_rules, result.recommended_action);
                process.exit(2);
            }
        ...
        case "Stop": {
            ...
            if (result.decision === "block") {
                writeBlockResponse(result.reason ?? "Blocked by governance runtime.");
                process.exit(2);
            }
...
function writeContextInjection(content: string) {
    process.stdout.write(JSON.stringify({ type: "context", content }) + "\n");
}
function writeBlockResponse(message: string, applied_rules?: string[], recommended_action?: string) {
    const parts = [message];
    ...
    process.stdout.write(JSON.stringify({ type: "block", message: parts.join("\n") }) + "\n");
}
function formatAdvisory(result: { reason?: string; applied_rules?: string[]; missing_evidence?: string[]; recommended_action?: string }) {
...
const isMain = process.argv[1] && (process.argv[1].endsWith("claude_code_adapter.js") ||
    process.argv[1].endsWith("claude_code_adapter.ts"));
if (isMain) {
    runClaudeHook().catch((err) => {
