...
const TOOLS = [
  {
    ...
    description:
      "Returns the current Procedural Context Pack — a markdown summary of the active case, ticket, branch, why this work exists, paused workstreams, and next expected action. This is the primary governance state query for AI agents.",
    inputSchema: {
  ...
  },
  {
    ...
    description:
      "Returns the active ticket as a structured JSON object. Includes ticket_id, revision, status, workstream_status, title, objective, acceptance_criteria, non_goals, risk_profile, and reason_for_reissue.",
    inputSchema: {
      type: "object" as const,
      properties: {
        cwd: {
  ...
  },
  {
    name: "gov_why",
    description:
      "Returns a docket-derived explanation of why the current work exists. Includes origin event, how the work evolved (reissues, deepening), and current state. Built entirely from procedural docket — no model inference.",
    inputSchema: {
      type: "object" as const,
      properties: {
        cwd: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
            ...
            content: [
              {
                type: "text" as const,
                text: JSON.stringify(
                  { error: "No active ticket", hint: "Run govctl ticket list" },
                  null,
                  2
                ),
              },
      ...
      case "gov_why": {
        const targetCaseId =
          (typeof inputArgs["case_id"] === "string" ? inputArgs["case_id"] : null) ??
...
function buildNarrative(
  ...
  const lines: string[] = ["# Why This Work Exists", ""];
  ...
  const refinements = events.filter(
    (e) =>
